...
export default function FinancialDashboard() {
  const [activeTimeframe, setActiveTimeframe] = useState("1_Hour");
  const [selectedTicker, setSelectedTicker] = useState(null);
  const [tickerData, setTickerData] = useState([]);
  const [loading, setLoading] = useState(true);

  const API_BASE =
    "https://backend-production-1e63.up.railway.app/api/tickers/data";

  useEffect(() => {
    const fetchTickerData = async () => {
      setLoading(true);
      try {
        const res = await fetch(`${API_BASE}/?timeframe=${activeTimeframe}`);
        const data = await res.json();
        setTickerData(data);
        setSelectedTicker(data?.[0] || null);
      } catch (err) {
    ...
    <div className="financial-dashboard">
          ...
          {tickerData.map((ticker, idx) => (
            <div
              ...
              className={`ticker-card ${
                selectedTicker?.symbol === ticker.symbol ? "selected" : ""
              }`}
              onClick={() => setSelectedTicker(ticker)}
            >
              <h3>{ticker.symbol}</h3>
              <p>{ticker.company}</p>
              <p className="price">{ticker.price}</p>
              <p
                className="change"
                style={{
                  color: ticker.change?.includes("-") ? "red" : "green",
                }}
              >
                {ticker.change}
              </p>
      ...
      {selectedTicker && (
        <div className="ticker-details">
          <h2>{selectedTicker.symbol} Details</h2>
          <p>
            Sentiment: {selectedTicker.sentiment} (
            {selectedTicker.sentiment_label})
          </p>
          <p>News Sentiment: {selectedTicker.news_sentiment}</p>
          <p>Contract: {selectedTicker.contract}</p>
          <p>Strike: {selectedTicker.strike}</p>
          <p>Contract Price: {selectedTicker.price_contract}</p>
          <p>Expiry: {selectedTicker.expiry}</p>
          <p>Volume: {selectedTicker.volume}</p>
          <p>ITM %: {selectedTicker.itm}</p>
