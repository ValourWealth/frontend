...
const OverallProgressBar = () => {
  ...
  useEffect(() => {
    ...
    fetch(
      "https://backend-production-1e63.up.railway.app/user/courses/progress/",
      {
        method: "GET",
        headers: {
      ...
      .then((data) => {
        ...
        const videoProgress =
          Math.round((data?.overall?.watched / data?.overall?.total) * 100) ||
          0;

        const courses = data?.courses || [];
        const levelProgress = courses.length
          ? Math.round(
              courses.reduce((acc, course) => acc + course.video_progress, 0) /
                courses.length
            )
          : 0;
